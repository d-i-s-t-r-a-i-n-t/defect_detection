...
for image_file in image_files:
    ...
    # пороговое значение для выделения объекта
    _, thresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # морфологическое расширение для создания области фона
    kernel = np.ones((5, 5), np.uint8)
    sure_bg = cv2.dilate(thresh, kernel, iterations=5)

    # трансформация расстояния, чтобы найти область переднего плана
    dist_transform = cv2.distanceTransform(thresh, cv2.DIST_L2, 5)
    _, sure_fg = cv2.threshold(dist_transform, 0.7 * dist_transform.max(), 255, 0)

    # граница между фоном и передним планом
    sure_fg = np.uint8(sure_fg)
    unknown = cv2.subtract(sure_bg, sure_fg)

    # маркировка области для разделения фона и объекта
    ...
    # объединение масок GrabCut и Watershed
    final_mask = mask2 & (watershed_mask // 255)

    # наложение объединенной маски на изображение
